...
public class EntregarChatCommandPlugIn : IChatCommandPlugIn
{
    ...
    public async ValueTask HandleCommandAsync(Player player, string command)
    {
        ...
        try
        {
            ...
            {
                var freeSlot = Enumerable.Range(0, maxSlot + 1).FirstOrDefault(s => !usedSlots.Contains(s));
                if (freeSlot == 0 && usedSlots.Contains(0))
                {
                    freeSlot = Enumerable.Range(1, maxSlot).FirstOrDefault(s => !usedSlots.Contains(s));
                }

                if (freeSlot > maxSlot || freeSlot < 0)
                {
